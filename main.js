...
var WS = {
	// settings
	serverHost: window.location.host, // automatically retrieved from the URL (ought to be a string)
	serverPort: 9300,
	...
	connect2Server: function() {
		...
		document.getElementById('info').innerHTML = 'Connecting..<br>';
		WS.connect();
	},
	...
	connect: function() {
		var Socket = new WebSocket('ws://'+WS.serverHost+':'+WS.serverPort);
		WS.setSocketEvents(Socket);
		WS.socket = Socket;
	},
	disconnect: function() {
		WS.socket.send('QUIT');
	...
	setSocketEvents: function(Socket) {
		Socket.onopen = function() {
			...
			WS.connected = true;
			document.getElementById('info').innerHTML += 'Connected.<br>';
		...
		Socket.onmessage = function(Message) {
			...
			if (typeof data.respond != 'undefined') {
				if (data.respond == 'info') {
					document.getElementById('info').innerHTML += data.info+'<br>';
				}
				if (data.respond == 'error') {
					document.getElementById('info').innerHTML += 'Error<br>';
					deleteCookie('session');
				}
				if (data.respond == 'init') {
					document.getElementById('info').innerHTML += 'Your IP-Address is '+data.info+'<br>';
					Menu.init();
					var session = getCookie('session');
					if (session != null) {

						var obj = {
							'task': 'init-cookie-user',
							'session': session
						}
						obj = JSON.stringify(obj);
						WS.socket.send(obj);
				...
				if (data.respond == 'join-attempt') {
					document.getElementById('info').innerHTML += data.info+'<br>';

					if (data.status == 1 || data.status == 3) { // indicates successfull login.
						if (typeof data.session != 'undefined') {
							setCookie('session', data.session, 1);
						}
						addClass(document.getElementById('logged-in-state'), 'active');
						document.getElementById('logout').style.display = 'inline-block';
					} else if (data.status == 5) {
						deleteCookie('session');
					}
				}
				if (data.respond == 'logout') {
					location.reload();
				}
			}
		...
		Socket.onclose = function() {
			...
			if (WS.connected) {
				...
				document.getElementById('info').innerHTML += 'Disconnected.<br>';
			}
			else {
				// was not connected to server before onclose was called
				document.getElementById('info').innerHTML = 'Failed to connect. Consider to run server.php.<br>';
			}
...
var Menu = {
	init: function () {
		...
		document.getElementById('btn').addEventListener('click', function(e) {
			...
			document.getElementById('info').innerHTML += 'Data is being processed...<br>';
		...
		document.getElementById('logout').addEventListener('click', function(e) {
			deleteCookie('session');
			removeClass(document.getElementById('logged-in-state'), 'active');
